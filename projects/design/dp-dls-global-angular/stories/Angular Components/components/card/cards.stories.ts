...
//     const totalCards = columns * rows;
//     const cardData = Array.from({ length: totalCards }, () => ({
...
export default {
  ...
  argTypes: {
    ...
    columnCombination: {
      description: 'Select column size combination',
      control: 'select',
      options: ['6,6', '4,8', '8,4', '3,9', '9,3', '2,10', '10,2'],
      defaultValue: '6,6',
    },
    rows: {
      description: 'Number of rows',
    ...
    buttonType: {
      ...
      options: ['mat-flat-button', 'mat-stroked-button'],
      defaultValue: 'mat-flat-button',
      if: { arg: 'actions', eq: 1 }, // Show only if actions == 1
    },
  },
} as Meta;
...
.example-card-footer {
  padding: var(--foundation-spacing-s);
}
...
.custom-combo { width: 100%; }
.custom-combo > div { height: 100%; display: flex; }
.custom-combo mat-card { flex: 1 0 auto; height: 100%; display: flex; flex-direction: column; }
</style>
`;
...
export const SimpleCards: StoryObj = {
  ...
  render: ({ appearance, columns, rows }) => {
    ...
    const cardData = Array.from({ length: totalCards }, (_, index) => ({
      ...
      subtitle: 'Subheading',
      content:
        'Explain more about the topic shown in the card title and subheading through short supporting text here.',
    }));

    return {
      template: `
        ${gridCSS}
        <div class="card-grid cols-${columns}">
          <mat-card *ngFor="let card of cardData" [appearance]="appearance">
            <mat-card-header>
              <mat-card-title>{{ card.title }}</mat-card-title>
              <mat-card-subtitle>{{ card.subtitle }}</mat-card-subtitle>
            </mat-card-header>
      ...
      `,
      props: { cardData, appearance, columns },
    };
  },
};
      ...
      template: `
        ...
        <div class="card-grid cols-${columns}">
            <mat-card *ngFor="let card of cardData" [appearance]="appearance">
              ...
              <mat-card-actions>
                <ng-container *ngIf="card.actions === 1">
                  <button *ngIf="card.buttonType === 'mat-flat-button'" mat-flat-button>Action 1</button>
                  <button *ngIf="card.buttonType === 'mat-stroked-button'" mat-stroked-button>Action 1</button>
                </ng-container>
                <ng-container *ngIf="card.actions === 2">
                  <button mat-stroked-button>Action 2</button>
...
export const CardWithFooterStory: StoryObj = {
  ...
  render: ({ appearance }) => {
    return {
      template: `
        ${gridCSS}
          <mat-card [appearance]="appearance" class="example-card">
            <mat-card-header>
              <mat-card-title>Card With Footer</mat-card-title>
            </mat-card-header>
            <mat-card-content>
              <p>The title above is part of the <strong>mat-card-header</strong></p>
              <p>This content you are reading is part of the <strong>mat-card-content</strong></p>
              <p>Below are the visible chips, which are part of the <strong>mat-card-footer</strong></p>
            </mat-card-content>
            <mat-card-footer>
              <mat-chip-set>
                <mat-chip>Chip 1</mat-chip>
...
export const CardWithStandardLayoutImage: StoryObj = {
  ...
  render: ({ appearance, columns, rows, buttonType, actions }) => {
    const totalCards = columns * rows;
    const cardData = Array.from({ length: totalCards }, () => ({
      image: './assets/card-img-1.png', // Adjust path if needed
      actions,
      buttonType,
    }));
      ...
      template: `
        ...
        <div class="card-grid cols-${columns}">
          <mat-card *ngFor="let card of cardData" [appearance]="appearance">
            ...
            <mat-card-actions *ngIf="card.actions != 0">
              <ng-container *ngIf="card.actions === 1">
                <button *ngIf="card.buttonType === 'mat-flat-button'" mat-flat-button>Action 1</button>
                <button *ngIf="card.buttonType === 'mat-stroked-button'" mat-stroked-button>Action 1</button>
              </ng-container>
...
export const CardWithLandscapeLayout: StoryObj = {
  name: 'Card Interactive: Landscape Layout with Image',
  args: {
    appearance: 'raised',
    columns: 2,
    rows: 1,
            ...
            <a href="#" class="mat-card-wrapper"
              ...
              <mat-card [appearance]="appearance" class="mat-mdc-card--landscape">
                ...
                <div>
                  ...
                  <mat-card-content>
                    <p>This card features a landscape orientation, ideal for wider content. To achieve this layout, include <strong>class="mat-mdc-card--landscape"</strong></p>
                  </mat-card-content>
                </div>
              </mat-card>
            </a>
